...
const CODE_LEN: usize = 15;
const SMALL_LEN: usize = 7;
...
impl Default for Tables {
    fn default() -> Self {
        Self {
            long_encode: [0; 1 << CODE_LEN],
            short_encode: [0; 1 << SMALL_LEN],
            decode: [0; 42183],
        }
    }
}

fn make_tables_safe() -> Box<Tables> {
    let mut tables: Box<Tables> = Box::new(Default::default());
    ...
    ranges
        ...
        .enumerate()
        .for_each(|(idx, code)| {
            tables.long_encode[idx] = code;
            tables.decode[code as usize] = idx as u16;
        });
    ...
    ranges
        ...
        .enumerate()
        .for_each(|(idx, code)| {
            tables.short_encode[idx] = code;
            tables.decode[code as usize] = idx as u16 | 0x8000;
        });
    tables
}
...
pub fn get_lookups() -> &'static Tables {
    LOOKUPS.get_or_init(make_tables_safe)
}
