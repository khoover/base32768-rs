...
pub fn resize_bytes<T>(in_buf: &[T], byte_size: usize, size: usize) -> Box<[ResizedByte]>
where
    T: Into<u16> + Copy,
{
...
mod test {
    ...
    fn resize_even_bytes_correctly() {
        let bytes = super::resize_bytes(
            &[242u8, 67u8, 167u8, 208u8, 253u8, 91u8, 156u8, 21u8],
            8,
            15,
        );
        let correct_bytes = vec![
            ...
            super::ResizedByte { bytes: 5, bits: 4 },
        ];
